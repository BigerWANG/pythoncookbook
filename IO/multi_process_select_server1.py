...
class Worker(multiprocessing.Process):
    ...
    def run(self):
        print "{} 开始socket服务".format(self.name)
        while True:  # 给抢到的socket加上锁
            ...
            for sock in rl:  # 对返回值rl判断是否有socket进来，如果有连接进来的话sock激活
                if sock == self.socket:  # 代表有新的链接进来了
                    ...
                    print "{} 有新连接进来了 ".format(self.name)
                    ...
                    try:
                        ...
                        if not data:
                            print "{} 什么都没收到，关闭客户端连接...".format(self.name)
                            sock.close()
                            self.rlist.remove(sock)
                            continue
                        print "{} received data [{}] from client {}".format(self.name, data.decode(), sock)
            ...
            for sock in wl:  # 当有可写事件发生的时候，代表可以给客户端返回数据，
                print "{} 当有可写事件发生的时候，".format(self.name)
...
def start_server():
    ...
    worker_list = [Worker(server, lock) for _ in range(multiprocessing.cpu_count())]
